...
export default function TourProgram() {
  ...
  const activeTour = tourId 
    ...
    : tours.find(t => t.status === 'active' && t.participants?.some(p => p.id === user?.id || p.email === user?.email));
      ...
      <div style={{ padding: '0 16px', marginTop: '24px' }}>
          {hasProgram ? (
              typeof activeTour.program === 'string' ? (
                  <div className="card rich-text-content" style={{ padding: '24px', borderRadius: '16px', boxShadow: '0 4px 12px rgba(0,0,0,0.04)', background: 'white' }} dangerouslySetInnerHTML={{ __html: activeTour.program }} />
                          ...
                          <div key={index} style={{ display: 'flex', gap: '16px', marginBottom: '32px', position: 'relative', zIndex: 1 }}>
                              ...
                              <div className="card" style={{ flex: 1, padding: '20px', margin: 0, borderRadius: '16px', boxShadow: '0 4px 12px rgba(0,0,0,0.04)' }}>
                                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                                      <span style={{ fontSize: '13px', fontWeight: 'bold', color: day.color || 'var(--primary)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{day.day}</span>
                                  ...
                                  <div className="rich-text-content" style={{ color: 'var(--text-muted)', marginBottom: '20px' }} dangerouslySetInnerHTML={{ __html: day.description || '' }} />
