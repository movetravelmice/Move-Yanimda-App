...
export default function ParticipantTransferManager({ tourId, participant, onClose }) {
  ...
  useEffect(() => {
    if (existingPart?.flights?.length > 0) {
       ...
       setFlightsInput([
         { id: Date.now(), type: 'Gidiş Uçuşu', airline: '', from: '', to: '', flightNo: '', date: '', time: '', pnr: '', ticketNo: '' }
       ]);
    }
  ...
  const handleSave = () => {
      const flightsToSave = flightsInput.filter(f => f.pnr || f.ticketNo || f.flightNo).map(f => ({
          type: f.type,
          airline: f.airline,
  ...
  const addFlight = () => setFlightsInput(p => [...p, { id: Date.now(), type: 'Gidiş Uçuşu', airline: '', from: '', to: '', flightNo: '', date: '', time: '', pnr: '', ticketNo: '' }]);
      ...
      return (
        <div className="card" style={{ padding: 0, overflow: 'hidden', position: 'relative', borderRadius: '16px', boxShadow: '0 8px 16px rgba(0,0,0,0.1)', marginBottom: '16px', animation: 'fadeIn 0.3s' }}>
            ...
            <div style={{ padding: '16px', display: 'flex', justifyContent: 'space-between', background: 'var(--surface)' }}>
                ...
                <div><div style={{ fontSize: '10px', color: 'var(--text-muted)' }}>TARİH</div><div style={{ fontSize: '13px', fontWeight: 'bold' }}>{flight.date || '-'} {flight.time || '-'}</div></div>
               ...
               <div key={flight.id} style={{ position: 'relative' }}>
                   ...
                   <div style={{ background: 'white', padding: '16px', paddingTop: flightsInput.length > 1 ? '36px' : '16px', borderRadius: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', marginBottom: '24px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                        ...
                        <div>
                            ...
                            <input type="text" value={flight.pnr} onChange={e => updateFlight(flight.id, 'pnr', e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid var(--primary)', outline: 'none', textTransform: 'uppercase', fontWeight: 'bold' }} placeholder="WX93AB" />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="time" value={flight.time} onChange={e => updateFlight(flight.id, 'time', e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid var(--border-color)', outline: 'none' }} />
                        </div>
                   </div>
               </div>
            ))}
